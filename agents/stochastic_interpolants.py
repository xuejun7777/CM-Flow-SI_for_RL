...
from bridger.utils.util import unsqueeze_xdim, indicator_function
from torch_ema import ExponentialMovingAverage

class StochasticInterpolants(nn.Module):
    def __init__(self, model_args):
        super(StochasticInterpolants, self).__init__()
    ...
    def b_loss(self, b_net, t, xt, x0, x1, z, cond):
        ...
        # print(partial_t.shape, z.shape, gamma_der.shape, b.shape)
        # b_reshape = b.flatten(-2)
        # partial_t_reshape = partial_t.flatten(-2)

        batch, xdim = b.shape
        gamma_der_reshape = unsqueeze_xdim(gamma_der, xdim)
        # print(gamma_der_reshape.shape)
        # z_reshape = z.flatten(-2)

        loss = 0.5 * torch.norm(b, dim=-1) ** 2 - torch.sum((partial_t + gamma_der_reshape * z) * b, dim=-1)
        return torch.mean(loss)
    ...
    def loss(self, state, action, device):
        nobs = state.to(device).float().flatten(1)
        naction = action.to(device).float()
        # print(naction.shape)
        ...
        if self.prior_model is None:
            ...
            with torch.no_grad():
                if self.prior_policy == 'cvae':
                    prior_action = self.prior_model.sample(cond=nobs)
                elif self.prior_policy == 'gaussian':
                    prior_action = self.prior_model.sample(cond=nobs)
                prior_action = prior_action.float().to(nobs.device).squeeze()
    ...
    def sample(self, x_prior, cond, recod_traj=False):
        """
